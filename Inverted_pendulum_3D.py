...
def leer_imus(dt):
    ...
    ang_z_euler1 = imu1.euler
    if ang_z_euler1 is not None and ang_z_euler1[0] is not None:
        ang_z_euler1_rel = np.radians(np.array(ang_z_euler1) - np.array(ang_z_euler1_0))
        ang_z = wrap_to_pi(ang_z_euler1_rel[0])
    else:
        ang_z = 0.0
    ...
    if quat1 is not None and not np.allclose(quat1, (0.0, 0.0, 0.0, 0.0)):
        ...
        euler1_rel0 = euler1 - euler1_0
        euler1_rel = np.array([wrap_to_pi(a) for a in euler1_rel0])
    else:
        euler1_rel = np.zeros(3)

    # Inclinación medida en el frame del bastón
    alpha_b, beta_b, theta = euler1_rel[0], euler1_rel[1], euler1_rel[2]

    # Yaw relativo bastón–plataforma
    delta_yaw = wrap_to_pi(ang_z - phi)

    # Rotar inclinación al frame de la plataforma
    c = np.cos(delta_yaw)
    s = np.sin(delta_yaw)

    alpha =  c * alpha_b - s * beta_b
    beta  =  s * alpha_b + c * beta_b
    ...
    #print(f"[IMU1] ang_z={np.degrees(ang_z):.2f}")
    print(f"[IMU2] phi={np.degrees(phi):.2f}, [IMU1] ang_z={np.degrees(ang_z):.2f}")
...
K = np.array([
    [-19.0,  -8.0,   0.0000, -120.0,  60.0,  -43.0,  -15.0,   0.0000, -80.0,   30.0],
    [ -8.0, -19.0,  -0.0000, -60.0,  120.0,  -15.0,  -43.0,  -0.0000,  -30.0,  80.0],
    [  0.0000,   0.0000,   0.0,    0.0000,   -0.0000,   0.0000,    0.0000,   0.0,   -0.0000,   -0.0000]
])
...
#K = np.array([
#    [0.0, 0.0000, 0.0000, -110.0, 40.0, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000],
#    [0.0, 0.0000, 0.0000, -40.0, 110.0, 0.0000, 0.0000, 0.0000,  0.0000, 0.0000],
...
def parada_segura():
    u_stop = [0.0, 0.0, 0.0]
    try:
        for _ in range(3):  # enviarlo varias veces por seguridad
            enviar_datos(u_stop)
            time.sleep(0.05)
        print("[STOP] u = [0, 0, 0] enviado")
    except Exception as e:
        print(f"[STOP] Error enviando parada: {e}")
...
try: 
    ...
    print("\n[CTRL+C] Interrupción detectada")
    parada_segura()

finally:
    parada_segura()
    ser.close()
    print("[FINAL] Puerto serial cerrado")
