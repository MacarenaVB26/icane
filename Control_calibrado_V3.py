...
# ==========================
def leer_imus(dt):
    global alpha, beta, alpha_dot, beta_dot, phi, phi_dot, prev_angles, prev_phi,euler1_deg
    ...
    # Debug
    print(f"[IMU1] α={np.degrees(alpha):.2f}° β={np.degrees(beta):.2f}° theta = {np.degrees(theta):.2f}°| α̇={alpha_dot:.3f} β̇={beta_dot:.3f}".encode("utf-8", "replace").decode("utf-8", "replace"))
    #print(f"[IMU2] φ={np.degrees(phi):.2f}° | φ̇={phi_dot:.3f}".encode("utf-8", "replace").decode("utf-8", "replace"))
...
#K = np.array([
#    [-19.2601, -7.2982,   0.0000, -592.0059, 131.2501, -44.8296, -15.8977, 0.0000, -166.8300, 37.1499],  #Fx
#    [-7.2982, -19.2601,   0.0000, -131.2501, 592.0059, -15.8977, -44.8296, 0.0000,  -37.1499, 166.8300], #Fy
#    [0.0000,    0.0000,   1.9370,    0.0000,   0.0000,   0.0000,   0.0000, 1.7144,  -0.0000,  -0.0000]   #Torque general
#])
K = np.array([
    [-10, -7.2982,   0.0000, -592.0059, 131.2501, -44.8296, -15.8977, 0.0000, -166.8300, 37.1499],  #Fx
    [-7, -19.2601,   0.0000, -131.2501, 592.0059, -15.8977, -44.8296, 0.0000,  -37.1499, 166.8300], #Fy
    [0.0000,    0.0000,   1.9370,    0.0000,   0.0000,   0.0000,   0.0000, 1.7144,  -0.0000,  -0.0000]   #Torque general
])
...
def transform_velocities(u):
    ...
    L = 0.15
    phi_c = phi
    eps_O = 30 * np.pi/180 #Revisar valores
    ...
    T_f = np.array([ [sigma1, sigma2, sigma3], [sigma4, sigma5, sigma6], [L, L, L]])
