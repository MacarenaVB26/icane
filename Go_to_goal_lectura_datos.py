...
def recibir_datos():
    global pos_x, pos_y, count, N, vel_M1, torque_M1, vel_M2, torque_M2, vel_M3, torque_M3, vel_x, vel_y, dt_N, flag
    if ser.in_waiting > 0:
        flag=1
        ...
        try:
            ...
            if len(val) != 12:
                print(f"[UART] Paquete inválido ({len(val)} val): '{data}'")
                return  # ignorar este paquete y esperar otro
            
            dt_N,pos_x,pos_y,vel_M1,torque_M1,vel_M2,torque_M2,vel_M3,torque_M3, vel_x, vel_y, count = map(float, val)
            #print(f"[DEBUG] Dato crudo: '{data}'")  # Para ver qué llega exactamente
            print(f"[POSICION] X:{pos_x:.2f}, Y:{pos_y:.2f}, dt Nucleo: {dt_N:.3f}")
            #print(data)
            #valores = data.split(",")
...
filename = f"datos_modelo_icane_s31.csv"
...
print(f"[CSV] Guardando en archivo: {filename}")
