...
euler0 = r0.as_euler('zyx', degrees=False) # Secuencia yxz
...
while True:
    ...
    if quat is not None and not np.allclose(quat, (0.0, 0.0, 0.0, 0.0)):
        ...
        euler = r.as_euler('zyx', degrees=False)  # Secuencia yxz
        ...
        print(f"[IMU1] Euler (deg): alpha={np.degrees(alpha):7.2f}, beta={np.degrees(beta):7.2f}, gamma={np.degrees(gamma):7.2f}")
        print(f"[IMU1] Euler default (deg): alpha={(euler_default[0]):7.2f}, beta={(euler_default[1]):7.2f}, gamma={(euler_default[2]):7.2f}")
    else:
